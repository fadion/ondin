...
Fixing still happens outside the review, on a branch off the frozen SHA/tag where there is one.

⚠️ **In this repo there is not** — the tree is deliberately unfrozen (`codebase-review/SKILL.md`
Phase 0), so a Critical is fixed on `main`, and the ledger entry records the SHA the fix landed at.
The pass that found it still returns findings only; fixing is a separate act.
...
Opens with the SHA the round started at and how the tree is held — frozen at a tag, in a worktree,
or **deliberately not frozen**, which is this repo's case — then the **pass log**, which is the
review's cursor: it's what makes "run the next pass" unambiguous, and the only thing a resumed
session needs to read.

**On an unfrozen tree the pass log carries a `SHA` column**, because "which commit did this pass
read" is no longer answerable from the header. That column is what triage uses to decide whether a
finding needs re-checking before it is ranked.
...
| Pass | SHA | Status | Findings (C/H/M/L/D) | Not covered |
| --- | --- | --- | --- | --- |
| 0  | `7b8483c` | done | — | coverage skipped (tool unavailable) |
| A1 | `7b8483c` | done | 2/5/3/1/2 | — |
| S1.1 | `e91a2f0` | **in progress** | | |
```
...
left blank. On an unfrozen tree the `SHA` is `git rev-parse --short HEAD` at dispatch.

Findings follow:

```markdown
## [S1-L5-02] Critical — <one-line claim>
path/to/file.ext:1284 · `fn commit_writes`
...
**The location line carries the enclosing item, not just `file:line`.** On an unfrozen tree a line
number is the perishable half of the reference and the symbol is the durable one — an insertion
above moves the number and leaves `fn commit_writes` exactly where it was. Name the enclosing
`fn` / `impl` / `struct` / test module; where there is none, quote the line of code or the `///`
heading it sits under. Do the same for every line ref inside **Evidence**.
...
1. **Append; never splice.** New findings go in at the `**Status:** open` → `---` → `## Rejected`
   ...
   ⚠️ **In this repo, prefer `Edit` at the boundary anchor.** A bash heredoc here has failed
   repeatedly on long bodies, and a finding block is long. If a heredoc is used anyway, write the
   block to the scratchpad with `Write` first and then `cat scratch >> review/findings.md` — the
   append is then one short command with no body to mangle.
2. **Never mutate the ledger from a shell script.** A PowerShell list-splice destroyed the ledger
   of the sibling project this method came from — 9,720 lines to 176 — because `$lines[0..$n]` yields `object[]`,
...
1. `git rev-parse HEAD`. On a frozen tree it must still match the ledger's SHA and a mismatch stops
   the loop. **On an unfrozen tree it is recorded, not asserted**: write it into the row, and if it
   moved since the previous pass, note which files changed
   (`git diff --name-only <prev>..HEAD -- crates/`) — a finding already raised against one of them
   is a candidate for re-checking at the next triage rather than a reason to stop.
...
6. **Guards, on return:** `git status --short` must match what it was at dispatch — capture it in
   step 4 and compare, rather than requiring it to be empty, since on an unfrozen tree the user may
   legitimately have work in progress. `review/` is gitignored, so any **new** entry means the
   subagent edited source or left a probe behind: revert that entry, record `attempt 2`, re-run.
   Then the §5 ledger checks.
...
and the ledger's header — including, on an unfrozen tree, the instruction to anchor every location
on its enclosing item and not on a line number alone. It appends its own findings block at the `## Rejected`
