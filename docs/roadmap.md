...
- ~~**A CSS selector needing a combinator**, which is a cascade engine rather than a lookup. Type,
  ...
  compound parser plus an ancestor walk, and what makes a real one large is everything this still
  refuses. `Css` reads `rect.a#b` and `*`, joined by whitespace or `>`, in any chain. What is left
  needs something other than an ancestor walk — `+` and `~` want sibling order, `:pseudo` wants
  state, `[attr]` wants attribute matching — and is still reported once for the sheet, so the
  module's *"nothing is lost silently"* contract is unchanged and covers a narrower set.
