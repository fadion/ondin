...
- **Is `char_valve`'s third arm reachable at all, and if not, delete it?** A maintainer question,
  and it replaces the "write the test" item that stood here until 2026-09-19. §15 D523 gave the Type
  panel's valve D316's engagement latch and kept a third arm nothing else in the app has — a
  `changed()` frame on a control that was **never engaged** — for the picker's **raw sensed
  regions**, on the reasoning that without it a click on the hue strip would commit nothing, ever.
  **The test was written and the reasoning was false** (§15 D802): `picker::pointer_slot` answers a
  click through `write_slot` before it would reach `valve_slot`, so the hue slider and the alpha
  strip never enter the valve on a click; a drag enters it as the engaged arm and its release as the
  falling edge, both arms the other two valves have; and on those frames `changed()` is never true.
  Measured by instrumenting all three arms and driving the click on a real picker. **Removing the arm
  still breaks nothing**, and now there is a reason. What is left is the ruling: `char_valve` has
  **seven** call sites, D802 measured one, and the only users anybody had written down are ruled out
  — so either delete the arm or find and document its real user. ⚠️ **§9.2's
  `clamp_existing_to_range(false)` argument does not settle it either way**: that turns on the arm's
  *predicate* being the shape of an egui-initiated rewrite, not on who arrives at it. ⚠️ **And the
  route that *is* taken now has a test** — `picker::text_colour_route_tests` — so nothing here is
  about coverage any more.
